...
class Assets {
						...
						case 'border':
							...
							if ( $border_style ) {
								foreach ( $border_directions as $direction ) {
									...
									if ( ! $border_color ) {
										$border_color = ! empty( $setting_value['color'] ) ? self::generate_css_color( $setting_value['color'] ) : 'var(--bricks-border-color)';
									}
									...
									$border_rules[ $direction ] = "$value $border_style $border_color";

									$border_widths[] = $value;
								}
							}

							if ( count( $border_rules ) ) {
								// All four border sides have same value: Use CSS shorthand
								if ( count( $border_widths ) === 4 && count( array_unique( $border_widths ) ) === 1 ) {
									$css_rules[ $css_selector ][] = "border: {$border_widths[0]} $border_style $border_color";
								} else {
									foreach ( $border_rules as $direction => $value ) {
										$css_rules[ $css_selector ][] = "border-$direction: {$value}";
									}
								}
							}

							// Only border-style and/or border-color set (no border-width)
							else {
								if ( ! empty( $setting_value['style'] ) ) {
									$css_rules[ $css_selector ][] = "border-style: {$setting_value['style']}";
								}
	...
	public static function generate_inline_css_from_element( $element, $controls, $css_type ) {
		...
		if ( $custom_css ) {
			// Line below no longer in use (@since 1.5) as it removes slash of 'content': "\2713"
